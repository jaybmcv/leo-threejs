...
let corridors=0;for(const [decks,suffix]of [[[1,2,3,4,5,16],'connected_service_corridors'],[[2,7,20],'special_area_connections']])for(const d of decks){const r=full.getObjectByName(`Deck_${d}_${suffix}`);assert.equal(r?.userData.polish?.revision,2);physicalPolish(r);corridors++;}
