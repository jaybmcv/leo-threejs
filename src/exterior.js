...
export function createExterior(root,{hullAt}) {
  const exterior=new T.Group();exterior.name='01_EXTERIOR_REFINED_V31';root.add(exterior);
  ...
  const white=mat('V31_Ceramic_satin',0xcbd1d5,.52,.18),navy=mat('V31_Thermal_navy',0x243244,.62,.2),edge=mat('V31_Edge_alloy',0x647480,.4,.55),black=mat('V31_Recess',0x111c29,.62,.3),seam=mat('V31_Panel_reveal',0x71808a,.75,.1),blue=mat('V31_Observation_glass',0x1d3b53,.27,.4),orange=mat('V31_Amber_markers',0xeb9d47,.5,.15),ink=mat('V31_Identity_ink',0x263b53,.65,.08);
  const ceramicPanel=mat('V31_Ceramic_access_panels',0xbfc8ce,.6,.15),thermalPanel=mat('V31_Thermal_service_panels',0x2b3c50,.65,.22),gasket=mat('V31_Glazing_seals',0x172330,.82,.05);
  const palePanel=mat('V31_Dorsal_service_covers',0xc5cdd2,.59,.17);
  const warm=new T.MeshStandardMaterial({name:'V31_Inhabited_windows',color:0x294554,roughness:.3,metalness:.2,emissive:0x506874,emissiveIntensity:.2});
  const engineGlow=new T.MeshStandardMaterial({name:'V31_Engine_idle_glow',color:0x527ca5,emissive:0x426d92,emissiveIntensity:.8,roughness:.25});
  ...
  function shell(s,p){
    ...
    // Evaluate the continuous roof normal across the fairing and forward crown.
    // Averaging the thin loft triangles leaves alternating highlights along
    // the roof rim, even though their positions follow the same smooth surface.
    const positions=g.attributes.position,normals=g.attributes.normal;
    for(let i=0;i<positions.count;i++){
      const x=positions.getX(i),y=positions.getY(i),z=positions.getZ(i);
      if(x<=-249||x>=299||y<=4)continue;
      const d=.1,n=V(-(roofY(x+d,z)-roofY(x-d,z))/(2*d),1,-(roofY(x,z+d)-roofY(x,z-d))/(2*d)).normalize();
      const forward=T.MathUtils.smoothstep(x,44,54);
      const heightWeight=(1-forward)*T.MathUtils.smoothstep(y,4,16)+forward*T.MathUtils.smoothstep(y,28,44);
      const weight=heightWeight*T.MathUtils.smoothstep(x,-249,-239)*(1-T.MathUtils.smoothstep(x,290,299));
      const blended=V(normals.getX(i),normals.getY(i),normals.getZ(i)).lerp(n,weight).normalize();
      normals.setXYZ(i,blended.x,blended.y,blended.z);
    }
  ...
  // Feather the forward inboard wing tip down into the broad lower hull.
  const wingSeat=(x,z)=>-6*T.MathUtils.smoothstep(x,-140,-72)*(1-T.MathUtils.smoothstep(Math.abs(z),86,105));
  ...
  for(const s of [-1,1]){
    ...
    const wingDraft=subdivide(wg,3),wa=wingDraft.attributes.position;for(let i=0;i<wa.count;i++)wa.setY(i,wa.getY(i)+wingDrop(wa.getZ(i))+wingSeat(wa.getX(i),wa.getZ(i)));const bentWing=mergeVertices(wingDraft,1e-4);bentWing.computeVertexNormals();mesh('Blended_double_delta',bentWing,white,fixed);wg.dispose();wingDraft.dispose();
    ...
    line('Wing_leading_edge',Array.from({length:33},(_,i)=>{const z=129-50*i/32;const x=-185+(129-z)*86/50;return [x,-1.5+wingDrop(z)+wingSeat(x,z),s*z];}),.15,seam,fixed);
  }
  ...
  // Bake the tail proportions, then straighten its leading profile with a
  // smooth height redistribution that preserves every X/Z vertex and the cap.
  ...
  tailGroup.traverse(o=>{if(!o.isMesh)return;o.updateMatrix();o.geometry.applyMatrix4(o.matrix);if(['Swept_tail_cap','Tail_top_amber'].includes(o.name))o.geometry.applyMatrix4(capForm);o.geometry.applyMatrix4(tailForm);o.geometry.translate(0,-32,0).scale(1,.92,1).translate(0,32,0);const a=o.geometry.attributes.position;for(let i=0;i<a.count;i++){const y=a.getY(i),t=T.MathUtils.clamp((y-36)/90,0,1);a.setY(i,y+7.5*Math.sin(Math.PI*t)**2);}o.geometry.computeVertexNormals();o.position.set(0,0,0);o.rotation.set(0,0,0);o.scale.set(1,1,1);o.updateMatrix();});
  ...
  exterior.userData={revision:'26',scope:'Short roof return and smooth aft deck with fixed outer plan',glazing:'Hull-conforming observation bands; pressure-shell engineering remains conceptual'};
