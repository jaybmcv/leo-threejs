...
export function encloseAftTour(root){
 const g=new T.Group();g.name='Tour_aft_shell';
 ...
 closedDoorX(g,A.x1,0,A.floor,-1);ceilingStrips(g,A.x0+3,A.x1-3,A.ceiling,[-6,0,5]);
 ...
 slab(g,'Tour_fin_gallery_wall',[F.x1,F.floor,-F.half-.2],[F.x1+.2,F.ceiling,F.half+.2],m.wall);
 closedDoorX(g,F.x1,0,F.floor,-1,1.4,2.4);ceilingStrips(g,F.x0+5,F.x1-1,F.ceiling,[-2,2]);
 return g;
}
...
function vestibule(g,m,{x,dir,zc,half,floor,ceiling,depth}){
 ...
 for(const s of [-1,1])slab(g,'Tour_vestibule_wall',[x0,floor,s>0?zc+half:zc-half-.2],[x1,ceiling,s>0?zc+half+.2:zc-half],m.wall);
 slab(g,'Tour_vestibule_end',[dir>0?x1:x0-.2,floor,zc-half-.2],[dir>0?x1+.2:x0,ceiling,zc+half+.2],m.wall);
