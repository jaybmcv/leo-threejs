...
async function loadExterior(){
  ...
  $('loading').hidden=true;
  stopTour();if(tourResidence)tourResidence.visible=false;if(tourTransit)tourTransit.visible=false;mode=next;document.body.dataset.eye='0';applyShapeView();document.body.dataset.mode=mode;persp.fov=mode==='walk'?65:42;persp.near=mode==='walk'?.05:mode==='neighborhood'?.1:1;transition=null;contextOpacity(1);
  ...
  try{
    ...
    if(location.protocol==='http:'&&['127.0.0.1','localhost'].includes(location.hostname)){
      ...
      if(view==='top'&&shapeOnly){
        ...
        const overlay=await new Promise(resolve=>renderer.domElement.toBlob(resolve,'image/png'));
